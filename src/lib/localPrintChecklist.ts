...
const BOOKING_SUMMARY_PRINT_STYLES = `<style id="cw-booking-summary-print-fix">
@media print {
  html, body {
    height: auto !important;
    overflow: visible !important;
  }
  .a4-page[data-print-doc="booking-summary"] {
    min-height: 0 !important;
    height: auto !important;
  }
  .summary-table thead {
...
export function injectBookingSummaryDocumentStyles(html: string): string {
  const withScreen = injectPreviewScreenStyles(html)
  if (/<\/head>/i.test(withScreen)) {
    return withScreen.replace(/<\/head>/i, `${BOOKING_SUMMARY_PRINT_STYLES}</head>`)
  }
  return BOOKING_SUMMARY_PRINT_STYLES + withScreen
}
